|slug|get-cursor-type|
|---|---|

# Get REF CURSOR data type

OceanBase Connector/ODBC supports the REF CURSOR data type in Oracle mode to implement cursor functionality.

In the following example, the `sys_refcursor` field indicates that the output data type is a cursor.
